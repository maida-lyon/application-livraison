  ...
  return (
    <div className="p-6 max-w-6xl mx-auto bg-white text-black rounded-xl shadow space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-full font-semibold transition ${mode === "freight" ? "bg-black text-white" : "bg-gray-200"}`}
          >
            FRET
          </button>
          <button
            onClick={() => setMode("distribution")}
            className={`px-4 py-2 rounded-full font-semibold transition ${mode === "distribution" ? "bg-black text-white" : "bg-gray-200"}`}
          >
            DISTRIBUTION
          </button>
        </div>
      ...
      <section className="bg-gray-50 rounded-xl p-4 shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <SignaturePad
            label="Signature chargement"
            name="signatureChargement"
            setForm={() => {}}
          />
          <SignaturePad
            label="Signature livraison"
            name="signatureLivraison"
            setForm={() => {}}
          />
        </div>
      </section>
